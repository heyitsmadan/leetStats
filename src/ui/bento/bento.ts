...
async function renderComponentContent(container: HTMLElement, selections: any, skillData: SkillMatrixData) {
    ...
    if (history && historyStartDate && historyEndDate) {
        ...
        if (card) {
            ...
            const bentoInteractiveFilters: InteractiveChartFilters = {
                ...
                timeRange: 'All Time', // This is effectively ignored because we provide a brushWindow
                difficulty: 'All', // This is a top-level filter, not used by the bento chart itself
    ...
    if (selectedRecords.length > 0) {
        ...
        if (card) {
            ...
            card.innerHTML = `<div class="bento-card-content"><div class="record-list">${listHtml}</div></div>`;
        }
    }
    ...
    if (selectedTrophies.length > 0) {
        ...
        if (card) {
            ...
            card.innerHTML = `<div class="bento-card-content"><div class="trophy-list">${listHtml}</div></div>`;
        }
    }
